...
const normalizeBigInt = (obj) => JSON.parse(JSON.stringify(obj, (_, value) => typeof value === 'bigint' ? Number(value) : value));
...
export const updateClientRating = async (data) => {
    const clientRequest = await prisma.clientRequests.findUnique({
        where: { id: data.id }
    });
    if (!clientRequest) {
...
export const getTimeAndDistance = async (originLat, originLng, destinationLat, destinationLng) => {
...
export const getNearbyClientRequests = async (driverLat, driverLng) => {
    try {
...
export const getByClientRequest = async (id) => {
    const rawData = await prisma.$queryRaw `
        SELECT
            CR.id,
            CR.id_client, 
...
export const getByClientAssigned = async (id_client) => {
    const rawData = await prisma.$queryRaw `
        SELECT
            CR.id,
            CR.id_client, 
...
export const getByDriverAssigned = async (id_driver_assigned) => {
    const rawData = await prisma.$queryRaw `
        SELECT
            CR.id,
            CR.id_client, 
...
//# sourceMappingURL=client_request.service.js.map