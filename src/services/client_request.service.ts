...
export const getTimeAndDistance = async (
    ...
    if (!countryConfig || !countryConfig.is_active) {
        console.warn(`⚠️ No se encontró configuración activa para '${targetCountryCode}'. Aplicando fallback por defecto en Dólares 'US'.`);
...
export const getNearbyClientRequests = async (driverLat: number, driverLng: number) => {
    try {
        ...
        try {
            ...
            console.warn("⚠️ Google Distance Matrix falló u omitió distancia exacta:", error);
        }
