...
// Helper para construir URLs absolutas válidas para imágenes (Soporta Railway y Local)
const buildImageUrl = (imagePath: string | null) => {
    ...
    // Si la app está desplegada en Railway (usa HTTPS y el dominio público de Railway)
    if (process.env.RAILWAY_PUBLIC_DOMAIN) {
        return `https://${process.env.RAILWAY_PUBLIC_DOMAIN}${imagePath.startsWith('/') ? '' : '/'}${imagePath}`;
    }

    // Si definiste PUBLIC_URL en las variables de entorno
    ...
    return `http://${host}:${port}${imagePath.startsWith('/') ? '' : '/'}${imagePath}`;
};
    ...
    try {
        ...
        const formatted = data.map((item: any, index: number) => {
            ...
            return {
                ...
                client: {
                    ...clientObj,
                    image: buildImageUrl(clientObj.image)
                },
...
export const getByClientRequest = async (id: number) => {
    ...
    const formatted = {
        ...
        driver: {
            ...driverObj,
            image: buildImageUrl(driverObj.image)
        },
    };
    return normalizeBigInt(formatted);
};
